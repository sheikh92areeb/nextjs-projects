            ...
            <AnimatePresence>
              {cartData.map((item,key)=> (
                <motion.div
                  ...
                  animate={{ opacity:1, y:0 }}
                  exit={{ opacity:0, y:-20 }}
                  className='flex flex-col sm:flex-row items-center bg-white rounded-2xl shadow-md p-5 hover:shadow-xl transition-all duration-200 border border-gray-100'
                >
                  ...
                  <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                    <button onClick={()=> dispatch(decreaseQuantity(item._id))} className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200'>
                      <Minus size={14} className='text-green-700' />
                    </button>
                    <span className='font-semibold text-gray-800 w-6 text-center'>{item.quantity}</span>
                    <button onClick={()=> dispatch(increaseQuantity(item._id))} className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200'>
                      <Plus size={14} className='text-green-700' />
                    </button>
                  </div>
                  <button onClick={()=>dispatch(removeFromCart(item._id))} className='sm:ml-4 mt-3 sm:mt-0 text-red-500 hover:text-red-700 transition-all'>
                    <Trash2 size={18} />
                  </button>
