...
import { Bike, ChevronRight, ChevronUp, CreditCard, MapPin, Package, Truck, UserCheck } from 'lucide-react'
    ...
    >
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 bg-linear-to-r from-green-50 to-white'>
        ...
        <div className='flex flex-wrap gap-2 items-center'>
          <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${order.isPaid ? "bg-green-100 text-green-700 border-green-300" : "bg-red-100 text-red-700 border-red-300"}`}>
      ...
      <div className='p-5 space-y-4'>
        ...
        {order.assignedDeliveryBoy && (
          <>
            ...
            <button onClick={() => router.push(`/user/track-order/${order._id?.toString()}`)} className='w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold px-4 py-2 rounded-xl shadow hover:bg-green-700 transition cursor-pointer'>
              <Truck size={18} />
              Track your order
            </button>
          </>
        ...
        <div className='border-t border-gray-200 pt-3'>
          <button onClick={()=>setExpended(prev=>!prev)} className='w-full flex justify-between items-center text-sm font-medium text-gary-700 hover:text-green-700 transition'>
        ...
        <div className='border-t pt-3 flex justify-between items-center text-sm font-semibold text-gray-800'>
          ...
          <div>
            Total: <span className='text-green-700 font-bold'>PKR {order.totalAmount}</span>
          </div>
        </div>
      </div>
    </motion.div>
